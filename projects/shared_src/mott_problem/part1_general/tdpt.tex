...
\newpage
\section{the Dyson series of the time evolution operator}
...
\begin{importantbox}
  \begin{align}
    {\cal A}_{fi}^{(1)} & =\sum_\gamma {c_\gamma^{(i)}}\langle \gamma_f|H_I|\gamma\rangle  \frac{\left(e^{-\frac{i}{\hbar}E_{\gamma_f}(t_f-t_i)}-e^{-\frac{i}{\hbar}{ E_\gamma}(t_f-t_i)} \right)}{{E_{\gamma_f}- E_\gamma}} \equiv \sum_\gamma {c_\gamma^{(i)}}\langle \gamma_f|H_I|\gamma\rangle   {\cal E}(E_{\gamma_f},E_{\gamma})\label{first_order_transition_amplitude}
  \end{align}
  with
  \begin{align}
  ...
  \end{align}
  The factor above is discussed in Sect \ref{sect-numerical-stable-first-order}
\end{importantbox}
