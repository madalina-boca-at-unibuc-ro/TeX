...
$$\langle P_f, n_f | H_I | P, 0 \rangle = \frac{V_0 \sigma_V }{\sqrt{2\pi}\hbar\sqrt{n_f!}} e^{-\frac{(P_f-P)^2}{2\hbar^2}\left(\sigma_V^2 + \frac{\alpha^2}{2}\right)} e^{-\frac{i}{\hbar}a_1(P_f-P)} \left( -\frac{i (P_f-P) \alpha}{\sqrt{2}\hbar} \right)^{n_f}$$
where:
...
| $\alpha$          | Oscillator length scale $\sqrt{\frac{(m \omega_0)}{\hbar}}$                                    | Absorbing atom      |
| $\alpha_{length}$ | Oscillator length scale $\sqrt{\frac{\hbar}{(m \omega_0)}}$; inverse of the $\alpha$ parameter | Absorbing atom      |
...
def compute_amplitude(Pf, nf, params):
    ...
    def matrix_element(P):
        pref = (V0 * sigma_V * np.sqrt(np.math.factorial(nf))) / (np.sqrt(2.0 * np.pi) * hbar)
