...
\end{align}
and the one absorber matrix elements are calculated in Appendix \ref{sec:osc-Vnm-displaced-basis}. The initial condition is given by the wave packet
\begin{align}
  \Psi_{n_1n_2}(X,0) = | 0(a_1)\rangle |0(a_2)\rangle|\psi_0(X,0)\rangle\delta_{n_1,0}\delta_{n_2,0}
\end{align}
i.e. both absorbers are in the ground state. The spatial part of the wave function is the same as in the one absorber case, given by \eqref{eq:proj-psi-alpha}.
