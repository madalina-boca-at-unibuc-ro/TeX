...
\begin{highlightbox}[title={Retarded proper time}]
    \begin{align}
...
\begin{highlightbox}[title={Lienard--Wiechert four-potential}]
    \begin{align}
...
\begin{highlightbox}[title={Covariant Faraday tensor}]
    \begin{align}
        F^{\alpha\beta}(x)
        ...
        {(u\cdot R_0)^3}\right|_{\tau=\tau_r}.\label{eq:F_alpha_beta_primary_form}
    \end{align}
\end{highlightbox}

Equivalently, the field tensor can be written in Jackson's form as
\begin{highlightbox}[title={Faraday tensor in Jackson's form}]
