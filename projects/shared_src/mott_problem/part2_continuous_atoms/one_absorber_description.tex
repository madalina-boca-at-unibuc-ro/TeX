\section{the model description; notations}
\label{sec:osc-one-absorber}
we consider the system consisting of the free alpha particle and one atom, described as a 1D harmonic oscillator, located at $x=a_1$.

the system Hamiltonian is
\begin{align}
  H=\frac{P_X^2}{2M}+\frac{P_x^2}{2m}+\frac{1}{2}m\omega_0^2(x-a_1)^2+V_0e^{-\frac{(x-X)^2}{2\sigma_V^2}}=H_0+H_I
\end{align}
where the unperturbed Hamiltonian is given by
\begin{align}
  H_0=\frac{P_X^2}{2M}+\frac{P_x^2}{2m}+\frac{1}{2}m\omega_0^2(x-a_1)^2
\end{align}
and the perturbation is given by
\begin{align}
  H_I=V_0e^{-\frac{(x-X)^2}{2\sigma_V^2}}
\end{align}
The energy eigenvectors of $H_0$ are
\begin{align}
  H_0|n(a_1),P\rangle = (\hbar\omega_0(n+1/2)+\frac{P^2}{2M})|n(a_1),P\rangle
\end{align}
in the position representations they are given by
\begin{align}
  \langle x,X|n(a_1),P\rangle = u_n(x-a_1)\frac{1}{\sqrt{2\pi\hbar}}e^{\frac{i}{\hbar}PX}
\end{align}
the initial state of the system in the position representation is given by
\begin{align}
  \langle x,X|\psi_i\rangle = u_0(x-a_1) \psi_\alpha(X)
\end{align}
where $\psi_\alpha(X)$ is the bimodal wave packet of the alpha particle with initial momentum $P_\alpha$ and initial width $\sigma_\alpha$, given in eq.~\eqref{eq:proj-psi-alpha}, and its momentum representation $C(P)$ and large-$P_\alpha$ approximation are given in eqs.~\eqref{eq:proj-CP}--\eqref{eq:proj-CP-approx} (Part~\ref{part:general-tdpt}, Sect.~\ref{sec:proj-wavepacket}); both are independent of the atom model and so are not re-derived here.
