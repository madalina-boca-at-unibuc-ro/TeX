\newpage
\section{first-order perturbative approach for two absorbers}\label{section-two-absorbers-o1}
...
\end{align}
here $C(P)$ is give by (\ref{C(P)-two-atom}) and the approximation (\ref{C(P)-two-atom-approx}).
\begin{importantbox}
  \begin{align}
    C(P) = \sqrt{\frac{\sigma_\alpha}{2\hbar\sqrt{\pi}\left(1+e^{-\frac{P_0^2\sigma_\alpha^2}{\hbar^2}}\right)}}\left[ e^{-\frac{(P-P_0)^2\sigma_\alpha^2}{2\hbar^2}} + e^{-\frac{(P+P_0)^2\sigma_\alpha^2}{2\hbar^2}} \right]\label{C(P)-two-atom-used-o1}
  \end{align}
  ...
  \begin{align}
    C(P) \approx \sqrt{\frac{\sigma_\alpha}{2\hbar\sqrt{\pi}}}\left[ e^{-\frac{(P-P_0)^2\sigma_\alpha^2}{2\hbar^2}}+ e^{-\frac{(P+P_0)^2\sigma_\alpha^2}{2\hbar^2}} \right]\label{C(P)-two-atom-approx-used-o1}
  \end{align}
\end{importantbox}
...
\begin{align}
  {\cal A}_{fi}^{(1)} & =\sum_\gamma {c_\gamma^{(i)}} \langle \gamma_f|H_I|\gamma\rangle  {\cal E}(E_{\gamma_f},E_\gamma)\label{first_order_transition_amplitude-used}
\end{align}
...
\begin{importantbox}
  \begin{align}
    \langle n_1(a) P_1|H_I|n_2(a) P_2\rangle={\cal V}(n_1,n_2; P_1, P_2; a)\label{one-absorber-matrix-element}
  \end{align}
\end{importantbox}
and they are calculated in Sect \ref{cal-V_olh-basis}
\begin{importantbox}
  \begin{align}
    {\cal V}(n_1,n_2;P_1,P_2;a)&= \langle n_1(a) P_1|H_I|n_2(a) P_2\rangle  \nonumber\\
    &= \frac{V_0}{2\pi\hbar}\sqrt{{2\pi\sigma_V^2}} e^{-\frac{i}{\hbar}a(P_1-P_2)}e^{-\frac{(P_1-P_2)^2\sigma_V^2}{2\hbar^2}}\sqrt{\frac{n_<!}{n_>!}} \left(-\frac{i (P_1-P_2)  }{\sqrt{2}\hbar\alpha}\right)^{n_> - n_<} e^{-\frac{(P_1-P_2)^2 }{4\hbar^2\alpha^2}} \, L_{n_<}^{(n_> - n_<)}\!\left(\frac{(P_1-P_2)^2 }{2\hbar^2\alpha^2}\right)\label{cal-V_n1n2p1p2-use}
  \end{align}
  ...
  \begin{align}
    ...
    \delta_{n_1n_2}.\label{cal-V_n1n2p1p2-spec-use}
  \end{align}
\end{importantbox}
...
\begin{importantbox}
  ...
  \end{align}
\end{importantbox}
the factor ${\cal E}$ is calculated in Sect. \ref{sect-numerical-stable-first-order} and is
\begin{importantbox}
  \begin{align} {\cal E}(E_{\gamma_f},E_\gamma)=\frac{e^{-i\tau E_{\gamma_f}}-e^{-i\tau E_\gamma}}{E_{\gamma_f}-E_\gamma} = -i\tau
    ...
    \right].\qquad \tau = \frac{t_f-t_i}{\hbar}\label{regularized-E-used}
  \end{align}
\end{importantbox}
...
\begin{importantbox}
  \begin{align}
    ...
    &= \frac{V_0}{2\pi\hbar}\sqrt{{2\pi\sigma_V^2}} e^{-\frac{i}{\hbar}a(P_f-P)}e^{-\frac{(P_f-P)^2\sigma_V^2}{2\hbar^2}}\sqrt{\frac{1}{n_f!}} \left(-\frac{i (P_f-P)  }{\sqrt{2}\hbar\alpha}\right)^{n_f} e^{-\frac{(P_f-P)^2 }{4\hbar^2\alpha^2}} \label{cal-V_n1n2p1p2-use-n0}
  \end{align}
  ...
  \begin{align}
    {\cal V}(n_f,0;P,P;a)
    = \frac{V_0}{2\pi\hbar}\sqrt{2\pi\sigma_V^2}\,
    \delta_{n_f0}.\label{cal-V_n1n2p1p2-spec-use-n0}
  \end{align}
  and $\alpha$ is
  \begin{align}
...
\begin{importantbox}
  ...
  \begin{enumerate}
    \item $M\gg m \sim 1$ au. The velocity $v$ of the alpha particle is of the order of 10 au (it has to be smaller that $c = 137.036$ au). In that case the momentum $P_0$ is very large; there will be two regions of interest for $P_f$, in two disconnected stripes around $\pm P_0$, with width of the order of $\sqrt{\frac{\hbar^2 }{\sigma_V^2 + \frac1{2\alpha^2}}}$
